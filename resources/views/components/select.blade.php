@props(['name', 'placeholder', 'disabled', 'dir', 'max', 'relative', 'onchange', 'relative', 'class', 'multiple'])
@php
    $defaults = [
        'name' => '',
        'placeholder' => 'Select',
...
<div ng-controller="DropdownController"
    ng-init="name='{{ $name }}'; dir='drop-{{ $dir }}'; max='{{ $max }}'; placeholder='{{ $placeholder }}'; relative='{{ $relative }}'; onchange='{{ $onchange }}';texts=['{{ $placeholder }}'];multiple = {{ $multiple?'true':'fakse' }};init()"
    class="inline-block">
